...
def getBalance():
  dfBalance = pd.DataFrame(exchange.fetch_balance(),columns=['USD','BTC','SOL/USD','free','used','total'])
...
def sendOrder(pair):                 # PRODUCT ที่เราต้องการเทรด ** # ไม่อยากใส่คู่ที่เทรดบ่อยๆ ใส่คู่ที่ต้องการเทรดแทน input เช่น pair = 'BTC-PERP'
  ...
  size_order = float(input('size'))                 # ใส่ขนาดเป็น BTC, ถ้า Rebalance ให้ใส่เป็น usd/price # แล้วไปกรอกในตัวแปร usd แทน
  price = int(input("Price:"))          # ระดับราคาที่ต้องการ
...
def checkOrder(pair):
       # ไม่อยากใส่คู่ที่เทรดบ่อยๆ ใส่คู่ที่ต้องการเช็คแทน input เช่น pair = 'BTC-PERP'            
    try:
      showPending(pair)
      print("     ")
      showMatched(pair)
